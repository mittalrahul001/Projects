...
<body>
    <?php if ($_SERVER['REQUEST_METHOD'] == 'POST'): ?>
       ...
       $createdByErr = '<span style="color: #f00;">Author Name is Required!</span>';
        ...
        <?php if ($formIsValid): ?>
            <?php
              $notesRepo = new \rmittal6\AS2\NoteRepository();
              $notes     = new \rmittal6\AS2\Notes();
              $count     = strlen($noteBody);
              $today     = date("F j, Y, g:i a"); //October 09, 2015, 1:16 pm
              $notes->setTitle($noteTitle);
              $notes->setBody($noteBody);
              $notes->setCreatedBy($createdBy);
              $notes->setCount($count);
              $notes->setCreatedAt($today);
    ...
    <div class="container">
        <section id = "content">
            ...
            <div>
            Last Modified At:
                <input type="text" name="date" value="<?php print $today?>" readonly>
            </div>
            <div>
